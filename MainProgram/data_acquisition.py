...
class DataAcquisition(threading.Thread):
    def __init__(self, list_of_variables_for_threads, bluetooth_server):
        ...
        self.config.sweep_rate = 30  # probably 30 is the best
    ...
    def tracking(self, data):
        ...
        if np.sum(power) > 1e-6: # TODO lägre värde?
        ...
        if self.first_data:
            ...
            if len(self.old_relative_distance_values) > 0:
                ...
                self.relative_distance = self.relative_distance - np.mean(self.old_relative_distance_values)/500

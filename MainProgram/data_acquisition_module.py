...
class DataAcquisition(threading.Thread):
    ...
    def tracking(self, data):
        ...
        if self.first_data:
            ...
            if self.max_peak_amplitude < (np.sum(amplitude[self.all_local_peaks_index])-self.max_peak_amplitude)/(len(self.all_local_peaks_index)-1)*4 or self.tracked_amplitude < 0.01:  # np.mean(amplitude)
                # Noise
